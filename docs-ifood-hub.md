...
O `iFood Hub` torna a plataforma a fonte de verdade de estoque e operacao por loja. Cada loja possui configuracao, credenciais e mapeamentos proprios; pedidos do iFood alimentam pedidos internos, estoque, kardex, faturamento operacional, alertas e auditoria em tempo real.
...
flowchart LR
  ...
  FN --> SEC["Secret Manager por loja"]
...
| Credenciais | Client ID, Client Secret e segredo futuro de webhook gravados no Google Secret Manager por loja |
...
| Catalogo e estoque | Importacao de produtos Catalog v2.0, mapeamento, baixa/estorno transacional, kardex e inventario oficial |
...
`quantity: 0` limita a venda a zero; uma reposicao publica novamente o saldo positivo. O campo opcional `inventoryEndpointTemplate` permite substituir a rota se um contrato iFood especifico exigir variante homologada, mas o padrao ja utiliza a rota oficial atual.
...
| --- | --- | --- |
| `ifood/config` | Merchant ID, flags, URLs, referencias a Secret Manager; sem leitura direta do browser | Function |
...
2. Obtenha `Merchant ID`, `Client ID` e `Client Secret`.
